...
describe('asking an appliance for its initial values', () => {
    test('the retry backs off instead of asking at a fixed rate for ever', (t) => {
        // An air conditioner that fell behind took seven hundred and eighty-seven packets
        // without once answering, while one that started cleanly took forty-two in total.
        // Asking every fifteen seconds for ever never gives a slow appliance a quiet moment.
        enableMockTimers(t)
        const { thinq, dev } = makeDevice()
        // The base class leaves both recognisers to the appliance handlers, and the harness
        // clears the capability retry the constructor sets up.
        dev.isCapsResponse = () => true
        dev.query_caps_timeout = setInterval(() => {}, 1e6)
        dev.processTLV([{ t: 0x2da, l: 0, v: 1 }] as never)
        thinq.resetRecorder()

        tickMockTimers(t, 15 * 1000)
        const afterFirst = thinq.outbox.length
        assert.equal(afterFirst, 1, 'the first retry comes at fifteen seconds')

        tickMockTimers(t, 15 * 1000)
        assert.equal(thinq.outbox.length, afterFirst, 'the next one does not')

        tickMockTimers(t, 15 * 1000)
        assert.equal(thinq.outbox.length, afterFirst + 1, 'it comes at thirty')
        dev.drop()
    })

    test('the retry stops as soon as the appliance answers', (t) => {
        enableMockTimers(t)
        const { thinq, dev } = makeDevice()
        dev.isCapsResponse = () => true
        dev.query_caps_timeout = setInterval(() => {}, 1e6)
        dev.processTLV([{ t: 0x2da, l: 0, v: 1 }] as never)
        dev.isValuesResponse = () => true
        dev.processTLV([{ t: 0x1f7, l: 0, v: 1 }] as never)
        thinq.resetRecorder()

        tickMockTimers(t, 10 * 60 * 1000)
        assert.deepEqual(thinq.outbox, [], 'nothing more is asked for')
        dev.drop()
    })
})
