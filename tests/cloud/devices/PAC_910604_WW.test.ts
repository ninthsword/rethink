...
describe('PAC_910604_WW', () => {
    test('exposes its live-confirmed controls and raw power value', () => {
        ...
        assert.equal(ha.devices[DEVICE_ID].properties['humidity_sensor_mode-'], 'operating_only')
        thinq.resetRecorder()
        ha.setProperty(DEVICE_ID, 'humidity_sensor_mode', 'command', 'always')
        ...
        thinq.resetRecorder()
        ha.setProperty(DEVICE_ID, 'humidity_sensor_mode', 'command', 'operating_only')
